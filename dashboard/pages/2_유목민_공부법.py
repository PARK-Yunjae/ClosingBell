...
if selected_stock_str:
    ...
    with col1:
        ...
        if selected_candidate.get('market'):
            ...
            st.write(f"• **섹터**: {selected_candidate.get('sector', '-')}")
            st.write(f"• **시가총액**: {selected_candidate.get('market_cap', 0)/100000000:.0f}억원" if selected_candidate.get('market_cap') else "• **시가총액**: -")
            st.write(f"• **PER**: {selected_candidate.get('per', '-')}")
            st.write(f"• **PBR**: {selected_candidate.get('pbr', '-')}")
            st.write(f"• **ROE**: {selected_candidate.get('roe', '-')}")
