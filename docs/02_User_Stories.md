# 2. 유저 스토리 및 수용 기준 (User Stories & Acceptance Criteria) v3.0
...
### 2.1.1 결론
14개의 유저 스토리를 우선순위별로 정의하고, 각 스토리에 대한 명확한 수용 기준을 설정하여 개발 완료 판단 기준을 명확히 한다.
...
#### US-01: 거래대금 필터링
**As a** 트레이더  
**I want** 당일 거래대금 100억 이상 종목만 필터링되기를
...
| ID | Given | When | Then |
|----|-------|------|------|
| AC-01-1 | 장 운영 중 (09:00~15:30) | 스크리닝 실행 시 | 거래대금 100억 미만 종목은 제외된다 |
| AC-01-2 | 거래대금 정확히 100억인 종목 | 스크리닝 실행 시 | 해당 종목은 포함된다 (≥ 조건) |
...
| ID | Given | When | Then |
|----|-------|------|------|
| AC-03-1 | 최근 2일 CCI 값이 연속 상승 | 기울기 분석 시 | 최고 가점 부여 |
| AC-03-2 | 최근 2일 CCI 값이 상승 추세 | 기울기 분석 시 | 높은 가점 부여 |
| AC-03-3 | 최근 2일 CCI 값이 횡보 | 기울기 분석 시 | 중립 점수 |
| AC-03-4 | 최근 2일 CCI 값이 하락 추세 | 기울기 분석 시 | 감점 부여 |
...
| ID | Given | When | Then |
|----|-------|------|------|
| AC-04-1 | MA20 최근 3일 연속 상승 | 기울기 분석 시 | 최고 가점 |
| AC-04-2 | MA20 최근 3일 상승 추세 | 기울기 분석 시 | 높은 가점 |
| AC-04-3 | MA20 횡보 (±0.5% 이내) | 기울기 분석 시 | 중립 점수 |
| AC-04-4 | MA20 하락 추세 | 기울기 분석 시 | 감점 |
| AC-04-5 | MA20 급락 (3일 -3% 이상) | 기울기 분석 시 | 강한 감점 |
...
| ID | Given | When | Then |
|----|-------|------|------|
| AC-11-1 | 스크리닝 완료 | 저장 시 | 모든 100억+ 종목 점수 저장 (TOP 3만 아님) |
