# 1. PRD (Product Requirements Document) v1.0
...
### 1.1.1 결론
한국투자증권 API를 활용하여 매일 장 마감 전 종가매매 후보 종목 TOP 3를 자동으로 선별하고, 디스코드/카카오톡으로 알림을 발송하며, 익일 결과를 추적하여 점수 가중치를 동적으로 최적화하는 시스템을 개발한다.
...
| 2 | CCI 기울기 | 상승세 여부 | 최근 3일 기울기 양수 = 가점 |
| 3 | MA(20일) 기울기 | 상승세 여부 | 최근 5일 기울기 양수 = 가점 |
| 4 | 양봉 품질 | 윗꼬리 짧음 + MA 위 안착 | 조건 충족 시 가점, 위반 시 감점 |
...
| DEP4 | 카카오 알림톡 설정 | ⏸️ 보류 (비즈니스 채널 필요) |
...
| Phase | 내용 | 예상 기간 |
|-------|------|----------|
| Phase 1 | 핵심 기능 (F1~F5) | 1주 |
| Phase 2 | 12:30 알림 + 익일 추적 (F6~F7) | 3일 |
| Phase 3 | 매매일지 + 대시보드 (F8~F9) | 1주 |
| Phase 4 | 카카오톡 연동 + 최적화 (F10) | 3일 |
