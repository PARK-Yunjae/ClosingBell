...
RSI_DEAD_ZONE_ENABLED = _env_bool("RSI_DEAD_ZONE_ENABLED", True)
RSI_DEAD_ZONE_LOW = _env("RSI_DEAD_ZONE_LOW", 40.0, float)
RSI_DEAD_ZONE_HIGH = _env("RSI_DEAD_ZONE_HIGH", 60.0, float)
RSI_DEAD_ZONE_LOOKBACK = _env("RSI_DEAD_ZONE_LOOKBACK", 10, int)
RSI_DEAD_ZONE_RATIO = _env("RSI_DEAD_ZONE_RATIO", 0.7, float)
OBV_DIVERGENCE_ENABLED = _env_bool("OBV_DIVERGENCE_ENABLED", True)
OBV_DIVERGENCE_LOOKBACK = _env("OBV_DIVERGENCE_LOOKBACK", 5, int)
OBV_DIVERGENCE_PRICE_THRESH = _env("OBV_DIVERGENCE_PRICE_THRESH", 2.0, float)
SCORE_OBV_BULLISH_BONUS = _env("SCORE_OBV_BULLISH_BONUS", 3.0, float)
SCORE_OBV_BEARISH_PENALTY = _env("SCORE_OBV_BEARISH_PENALTY", 3.0, float)
...
SCORE_CCI_SLOPE = _env("SCORE_CCI_SLOPE", 8.0, float)
SCORE_MA20_SLOPE = _env("SCORE_MA20_SLOPE", 7.0, float)
SCORE_RSI = _env("SCORE_RSI", 10.0, float)
...
VOL_BURST_OPTIMAL = (
    ...
    _env("VOL_BURST_OPTIMAL_HIGH", 8.0, float),
)
VOL_BURST_ZERO_HIGH = _env("VOL_BURST_ZERO_HIGH", 20.0, float)

WATCHLIST_MAX_DAYS = _env("WATCHLIST_MAX_DAYS", 5, int)
WATCHLIST_MAX_STOCKS = _env("WATCHLIST_MAX_STOCKS", 5, int)
DAILY_PICK_TOP_K = _env("DAILY_PICK_TOP_K", 5, int)
WATCHLIST_ALLOWED_RANKS = tuple(
    _env_int_list("WATCHLIST_ALLOWED_RANKS", [1, 2, 3, 4, 5])
)
...
RANK4_SWEET_SPOT = _env("RANK4_SWEET_SPOT", 3, int)
RANK4_WINDOW_START = _env("RANK4_WINDOW_START", 2, int)
RANK4_WINDOW_END = _env("RANK4_WINDOW_END", 4, int)
RANK5_SWEET_SPOT = _env("RANK5_SWEET_SPOT", 2, int)
RANK5_WINDOW_START = _env("RANK5_WINDOW_START", 1, int)
RANK5_WINDOW_END = _env("RANK5_WINDOW_END", 3, int)

RANK1_PULLBACK_BONUS = _env("RANK1_PULLBACK_BONUS", 5.0, float)
RANK2_PULLBACK_BONUS = _env("RANK2_PULLBACK_BONUS", 0.0, float)
RANK3_PULLBACK_BONUS = _env("RANK3_PULLBACK_BONUS", 0.0, float)
RANK4_PULLBACK_BONUS = _env("RANK4_PULLBACK_BONUS", -5.0, float)
RANK5_PULLBACK_BONUS = _env("RANK5_PULLBACK_BONUS", -5.0, float)
...
PULLBACK_MA5_GAP = _env("PULLBACK_MA5_GAP", 1.0, float)
PULLBACK_VOL_DECLINE = _env("PULLBACK_VOL_DECLINE", 0.5, float)
PULLBACK_BB_LOWER = _env("PULLBACK_BB_LOWER", 0.3, float)
BUY_A_MIN_SCORE = _env("BUY_A_MIN_SCORE", 68.0, float)
BUY_B_MIN_SCORE = _env("BUY_B_MIN_SCORE", 45.0, float)
BUY_MIN_CONDITIONS = _env("BUY_MIN_CONDITIONS", 2, int)
...
DISCORD_SCREEN_TOP_N = _env("DISCORD_SCREEN_TOP_N", 5, int)
DISCORD_PICK_TOP_N = _env("DISCORD_PICK_TOP_N", 5, int)
NOTIFIER_RECENT_SESSIONS = _env("NOTIFIER_RECENT_SESSIONS", 20, int)

PERFORMANCE_TRACK_DAYS = _env("PERFORMANCE_TRACK_DAYS", 5, int)

TOP_N = _env("TOP_N", 5, int)
TOP_N_CONSERVATIVE = _env("TOP_N_CONSERVATIVE", 3, int)
...
MIN_CHANGE_RATE = _env("MIN_CHANGE_RATE", -30.0, float)
MAX_CHANGE_RATE = _env("MAX_CHANGE_RATE", 30.0, float)
MIN_TRADING_VALUE = _env("MIN_TRADING_VALUE", "1000")
UNIVERSE_TOP_N = _env("UNIVERSE_TOP_N", 150, int)
UNIVERSE_MODE = _env("UNIVERSE_MODE", "union")  # v4: 항상 합집합 (Core/Fringe 태깅으로 대체)
...
FOREIGN_EXHAUST_WARN = _env("FOREIGN_EXHAUST_WARN", 90.0, float)
FOREIGN_EXHAUST_STRONG = _env("FOREIGN_EXHAUST_STRONG", 70.0, float)
SUPPLY_CAUTION_PENALTY = _env("SUPPLY_CAUTION_PENALTY", 3.0, float)
SUPPLY_DANGER_PENALTY = _env("SUPPLY_DANGER_PENALTY", 7.0, float)
SUPPLY_DANGER_THRESH = _env("SUPPLY_DANGER_THRESH", -5, int)
SUPPLY_CAUTION_THRESH = _env("SUPPLY_CAUTION_THRESH", -3, int)
SUPPLY_GOOD_BONUS = _env("SUPPLY_GOOD_BONUS", 3.0, float)
SUPPLY_GOOD_THRESH = _env("SUPPLY_GOOD_THRESH", 3, int)

# ── 외신 체커 (foreign_news_checker.py) ──
FOREIGN_NEWS_ENABLED = _env_bool("FOREIGN_NEWS_ENABLED", True)
...
FOREIGN_NEWS_MACRO_MODE = _env_bool("FOREIGN_NEWS_MACRO_MODE", True)
FOREIGN_NEWS_MACRO_MAX_QUERIES = _env("FOREIGN_NEWS_MACRO_MAX_QUERIES", 3, int)

# ── 유튜브 체커 (youtube_checker.py) ──
YOUTUBE_CHECK_ENABLED = _env_bool("YOUTUBE_CHECK_ENABLED", True)
...
YOUTUBE_THEME_MODE = _env_bool("YOUTUBE_THEME_MODE", True)
YOUTUBE_THEME_MAX_THEMES = _env("YOUTUBE_THEME_MAX_THEMES", 3, int)
