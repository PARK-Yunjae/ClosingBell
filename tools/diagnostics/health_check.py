...
from storage import list_screen_dates, list_watchlists
...
def check_data() -> list[tuple[str, str, str]]:
    ...
    # (3) 스크리닝 로그
    log_dates = list_screen_dates()
    if log_dates:
        last_log = log_dates[-1]
        l_gap = _days_since(last_log)
        status = OK if l_gap <= 1 else (WARN if l_gap <= 3 else FAIL)
        results.append((status, "스크리닝 로그", f"최신: {last_log} ({len(log_dates)}일분)"))
    else:
    ...
    # (5) 워치리스트
    watchlists = list_watchlists(desc=True)
    if watchlists:
        active = 0
        today = datetime.now().strftime("%Y-%m-%d")
        for wd in watchlists:
            if wd.get("expires", "") >= today:
                active += 1
        results.append((OK, "워치리스트", f"총 {len(watchlists)}개, 활성 {active}개"))
    else:
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="ClosingBell health check")
    ...
    if args.data:
        ...
        screen_t = SCHEDULE.get("screen", "15:40")
