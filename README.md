...
**종가매매 스크리너**는 매일 장 마감 전 **거래대금 300억 이상** 종목 중 5가지 기술적 지표를 분석하여 **종가매매 후보 TOP 3**를 선정하고 **Discord + 카카오톡**으로 알림을 발송하는 시스템입니다.
...
| 📱 **실시간 알림** | Discord + 카카오톡 (12:30 프리뷰 + 15:00 본알림) |
...
| **CCI 기울기** | CCI 5일 변화량 | 상승세 유지 | 0~10점 |
| **MA20 기울기** | 20일 이평선 기울기 | 우상향 추세 | 0~10점 |
...
과거 데이터로 전략 성과를 검증할 수 있습니다.

### 기본 사용법

```bash
# 기간 백테스트
python tools/run_backtest.py --start 2024-01-01 --end 2024-12-31

# TOP N 개수 지정 (기본값: 3)
python tools/run_backtest.py --start 2024-01-01 --end 2024-06-30 --top 5

# 상세 거래 내역 출력
python tools/run_backtest.py --start 2024-01-01 --end 2024-03-31 --verbose
```
...
Discord 외에 카카오톡으로도 알림을 받을 수 있습니다.
...
3. **REST API 키** 확인
4. **카카오 로그인** 활성화
5. **Redirect URI** 등록: `http://localhost:8080/callback`
6. **동의 항목** 설정:
   - `talk_message` (나에게 메시지 보내기) 권한 추가

### 2. 토큰 발급

```bash
# 토큰 발급 도구 실행
python tools/kakao_token_helper.py

# 안내에 따라:
# 1. 브라우저에서 카카오 로그인
# 2. 권한 동의
# 3. Redirect URL의 code 파라미터 복사
# 4. 토큰 발급 완료
```
...
KAKAO_ACCESS_TOKEN=발급받은_액세스_토큰
KAKAO_REFRESH_TOKEN=발급받은_리프레시_토큰
```
...
│   └── kakao_token_helper.py  # 카카오 토큰 발급
├── data/                  # 데이터 저장
├── logs/                  # 로그 파일 (일별 자동 분리)
└── docs/                  # 설계 문서
```
...
- **HTTP 클라이언트**: requests
- **대시보드**: Streamlit (옵션)
- **외부 API**: 한국투자증권 REST API, Discord Webhook, 카카오 REST API
...
```
logs/
├── 2026-01-07.log      # 오늘 로그
├── 2026-01-06.log      # 어제 로그
├── errors.log          # 에러 전용 로그
└── ...                 # 30일 보관
```
...
3. **카카오 토큰**: 액세스 토큰은 6시간, 리프레시 토큰은 2개월 유효합니다. 주기적으로 갱신이 필요할 수 있습니다.

4. **장 운영일 확인**: 공휴일에는 스케줄러가 자동으로 스킵합니다.
...
**설계 철학:** 뉴스 수집은 제외합니다. 모든 시장 정보(기대감, 내부자 정보 등)는 결국 차트에 반영된다고 봅니다. 닷컴버블(2000), IMF(1998), 금융위기(2008), 코로나(2020) 등 역사적 사건도 결국 가격과 거래량에 선반영되었습니다.
