...
# 기울기 계산 기간 (v2.2.1 패치: HTS 조건식 테스트 반영)
CCI_SLOPE_PERIOD = 2   # CCI 기울기 계산 기간 (최근 2일) - 단기 모멘텀
MA20_SLOPE_PERIOD = 3  # MA20 기울기 계산 기간 (최근 3일) - 추세 확인
...
class DefaultWeights:
    """기본 가중치"""
    cci_value: float = 1.0
    cci_slope: float = 2.5    # 중요
    ma20_slope: float = 2.5   # 중요
    candle: float = 2.5       # 중요
    change: float = 0.0       # 비활성화 (백테스트 결과 역효과)
...
CCI_SCORE_RANGES = [
    # (min, max, score) - 180에 가까울수록 높은 점수 ★ v2.2 재조정
    (175, 185, 10.0),  # 최고점 (180 정중앙) ★
    (165, 175, 9.0),   # 매우 좋음
    (185, 195, 9.0),   # 매우 좋음
    (150, 165, 8.0),   # 좋음
    (195, 210, 7.5),   # 좋음 (약간 과열 시작)
    (130, 150, 6.5),   # 중상
    (210, 230, 5.5),   # 중간 (과열 주의)
    (100, 130, 5.0),   # 중간
    (230, 280, 4.0),   # 낮음 (과열)
    (50, 100, 3.5),    # 낮음 (모멘텀 부족)
    (280, 350, 2.5),   # 감점 (고점 경고)
    (0, 50, 2.0),      # 감점 (저점)
]
...
# ============================================================
# 필터링 상수 (★ v3.2 백테스트 최적화)
# ============================================================

# 거래대금 필터 (억원)
MIN_TRADING_VALUE = 100  # v3.1: 100억 이상

# ★ 백테스트 최적 필터 (v3.2)
MAX_CHANGE_RATE = 15.0     # 등락률 15% 이하 (15%+ 승률 급락)
MIN_CHANGE_RATE = 4.0      # 등락률 4% 이상 (너무 약한 상승 제외)
MAX_CCI_FILTER = 190       # CCI 190 이하 (과열 제외)
MAX_MA20_DISTANCE = 8.0    # MA20 이격도 8% 이하 (30%+ 쪽박 위험)
MAX_CONSECUTIVE_UP = 4     # 연속상승 4일 이하 (5일+ 승률 38%)
CANDLE_SCORE_MIN = 6       # 양봉품질 6점 이상
CANDLE_SCORE_MAX = 8       # 양봉품질 8점 이하 (10점 완벽양봉 피하기)
...
# 스크리닝 결과
TOP_N_COUNT = 5  # v3.2: TOP 5 선정 (백테스트에서 TOP3~5 성과 좋음)
