"""
ClosingBell 앱 전역 설정
- 버전 정보
- 데이터 경로 (절대경로)
- 표시 문구

v7.0 - 키움 REST API 전환
"""

from pathlib import Path
import os

# ============================================================
# 버전 정보
# ============================================================
APP_VERSION = "v7.0"
APP_NAME = "ClosingBell"
AI_ENGINE = "Gemini 2.5 Flash"
BROKER_API = "키움증권 REST API"

# 전체 버전 문자열
APP_FULL_VERSION = f"{APP_NAME} {APP_VERSION}"

# ============================================================
# 데이터 경로 (절대경로 - 환경변수 또는 기본값)
# ============================================================
# Windows 로컬 환경 기본값
DEFAULT_DATA_DIR = r"C:\Coding\data"

# 환경변수에서 로드 (없으면 기본값)
DATA_DIR = Path(os.getenv("DATA_DIR", DEFAULT_DATA_DIR))
OHLCV_DIR = DATA_DIR / "ohlcv_kiwoom"  # v7.0: 키움 기반
OHLCV_FULL_DIR = DATA_DIR / "ohlcv"  # 3년+ 전체 데이터
GLOBAL_DIR = DATA_DIR / "global"
MAPPING_FILE = DATA_DIR / "stock_mapping.csv"
GLOBAL_MERGED_FILE = DATA_DIR / "global_merged.csv"

# ============================================================
# 표시 문구 템플릿
# ============================================================
# 대시보드 푸터
FOOTER_DASHBOARD = f"{APP_FULL_VERSION}"
FOOTER_WITH_SOURCE = f"{APP_FULL_VERSION} | {{source}} | {AI_ENGINE}"

# 유목민 공부법 푸터
FOOTER_NOMAD = f"{APP_FULL_VERSION} | 유목민 공부법 | 네이버 금융 + DART + {AI_ENGINE}"

# TOP5 푸터
FOOTER_TOP5 = f"{APP_FULL_VERSION} | D+1 ~ D+20 수익률 분석"

# 종목 검색 푸터
FOOTER_SEARCH = f"{APP_FULL_VERSION} | 종목 상세 분석"

# Discord 푸터
FOOTER_DISCORD = f"{APP_FULL_VERSION}"

# ============================================================
# 사이드바 설정
# ============================================================
SIDEBAR_TITLE = f"🔔 {APP_NAME}"
SIDEBAR_VERSION = APP_VERSION

# ============================================================
# 페이지별 제목
# ============================================================
PAGE_TITLES = {
    "home": "🏠 홈",
    "top5": "📊 종가매매 TOP5",
    "nomad": "📚 유목민 공부법",
    "search": "🔍 종목 검색",
}

# ============================================================
# 데이터 소스 표시
# ============================================================
DATA_SOURCES = {
    "kiwoom": "키움증권 REST API",  # v7.0
    "naver": "네이버 금융",
    "dart": "DART 전자공시",
    "ai": AI_ENGINE,
}

# ============================================================
# UI 설정
# ============================================================
# 유목민 종목 카드 열 수
NOMAD_CARD_COLUMNS = 3  # 기본 3열 (반응형으로 조절)

# TOP5 카드 열 수
TOP5_CARD_COLUMNS = 5

# 상세 분석 탭
DETAIL_TABS = ["기업정보", "뉴스", "AI 분석"]

# ============================================================
# 메시지 상수
# ============================================================
MSG_COMPANY_INFO_AUTO = "기업정보는 매일 자동 수집됩니다."
MSG_NO_PER = "PER 미제공 (적자 또는 신규상장)"
MSG_NO_PBR = "PBR 미제공"
MSG_NO_ROE = "ROE 미제공"

# 리스크 관련
MSG_AUDIT_OK = "감사의견: 적정 ✅"
MSG_AUDIT_WARNING = "감사의견: 확인 필요 ⚠️"
MSG_AUDIT_DANGER = "감사의견: 위험 🚨"

# ============================================================
# 색상 (Streamlit/Discord)
# ============================================================
COLORS = {
    "success": "#28a745",
    "warning": "#ffc107", 
    "danger": "#dc3545",
    "info": "#17a2b8",
    "primary": "#007bff",
    
    # 등급별 색상
    "grade_s": "#FFD700",  # 금색
    "grade_a": "#C0C0C0",  # 은색
    "grade_b": "#CD7F32",  # 동색
    "grade_c": "#808080",  # 회색
    "grade_d": "#A0A0A0",  # 연회색
    
    # 상한가/하한가
    "limit_up": "#FF0000",
    "limit_down": "#0000FF",
}

# ============================================================
# 유틸리티 함수
# ============================================================
def get_footer(source: str = None) -> str:
    """소스 포함 푸터 문자열 반환"""
    if source:
        return FOOTER_WITH_SOURCE.format(source=source)
    return FOOTER_DASHBOARD


def get_version_display() -> str:
    """버전 표시용 문자열"""
    return f"{APP_NAME} {APP_VERSION}"


# ============================================================
# 디버그용
# ============================================================
if __name__ == "__main__":
    print(f"앱 버전: {APP_FULL_VERSION}")
    print(f"AI 엔진: {AI_ENGINE}")
    print(f"데이터 디렉토리: {DATA_DIR}")
    print(f"OHLCV 디렉토리: {OHLCV_DIR}")
    print(f"매핑 파일: {MAPPING_FILE}")
    print()
    print(f"푸터 (대시보드): {FOOTER_DASHBOARD}")
    print(f"푸터 (유목민): {FOOTER_NOMAD}")
    print(f"푸터 (TOP5): {FOOTER_TOP5}")
