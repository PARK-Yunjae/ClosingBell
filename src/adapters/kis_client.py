...
class KISClient:
    ...
    def get_top_trading_value_stocks(
        ...
        Returns:
            종목 정보 리스트
        """
        # volume-rank API 사용 (보완 로직 제거 - v6.4)
        # 종목이 적으면 적은 대로 진행
    ...
    def get_investor_trend(
        ...
        Returns:
            투자자별 매매동향 리스트
            [{'date': date, 'foreign_net': int, 'inst_net': int, 'retail_net': int}, ...]
        """
        ...
        try:
            ...
            for item in output[:days]:
                try:
                    ...
                    results.append({
                        'date': dt,
                        'foreign_net_qty': int(item.get("frgn_ntby_qty", 0)),      # 외국인 순매수 수량
                        'foreign_net_amt': int(item.get("frgn_ntby_tr_pbmn", 0)),  # 외국인 순매수 금액
                        'inst_net_qty': int(item.get("orgn_ntby_qty", 0)),         # 기관 순매수 수량
                        'inst_net_amt': int(item.get("orgn_ntby_tr_pbmn", 0)),     # 기관 순매수 금액
                        'retail_net_qty': int(item.get("prsn_ntby_qty", 0)),       # 개인 순매수 수량
                        'retail_net_amt': int(item.get("prsn_ntby_tr_pbmn", 0)),   # 개인 순매수 금액
                        'close': int(item.get("stck_clpr", 0)),                    # 종가
                        'volume': int(item.get("acml_vol", 0)),                    # 거래량
                    })
                except (ValueError, KeyError) as e:
                    logger.warning(f"투자자 동향 파싱 오류 ({stock_code}): {e}")
                    continue
            
            return results
            
        except Exception as e:
            logger.error(f"투자자별 매매동향 조회 실패 ({stock_code}): {e}")
            return []
    
    def get_sector_investor_trend(
        self,
        sector_code: str = "0001",  # 0001: 코스피, 1001: 코스닥
        days: int = 5,
    ) -> List[Dict]:
        """시장/섹터별 투자자 매매동향 조회
        
        Args:
            sector_code: 시장코드 (0001: 코스피, 1001: 코스닥)
            days: 조회 일수
            
        Returns:
            시장별 투자자 매매동향
        """
        endpoint = "/uapi/domestic-stock/v1/quotations/inquire-daily-trade"
        tr_id = "FHKST03010200"
        
        params = {
            "FID_COND_MRKT_DIV_CODE": sector_code,
            "FID_INPUT_DATE_1": (datetime.now() - timedelta(days=days*2)).strftime("%Y%m%d"),
        ...
        try:
            ...
            for item in output[:days]:
                try:
                    ...
                    results.append({
                        'date': dt,
                        'foreign_net': int(item.get("frgn_ntby_tr_pbmn", 0)),  # 외국인 순매수 금액
                        'inst_net': int(item.get("orgn_ntby_tr_pbmn", 0)),     # 기관 순매수 금액
                        'retail_net': int(item.get("prsn_ntby_tr_pbmn", 0)),   # 개인 순매수 금액
                    })
                except (ValueError, KeyError) as e:
                    logger.warning(f"시장 투자자 동향 파싱 오류: {e}")
                    continue
            
            return results
            
        except Exception as e:
            logger.error(f"시장별 투자자 매매동향 조회 실패: {e}")
            return []
