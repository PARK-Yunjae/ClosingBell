...
🔧 v5.1 변경사항:
  - CCI 160~180 전체 구간 만점 (기존: 170 중심)
  - 연속양봉 5일 이상 감점 강화
  - Discord 표시에 CCI, 거래량비, 이격도 등 핵심지표 추가
"""
...
# ============================================================
# v5.1 점수 계산 함수들 (각 15점 만점) - 개선됨
# ============================================================

def calc_cci_score(cci: float) -> float:
    """CCI 점수 (15점 만점)
    
    v5.1 개선: 160~180 전체 구간 만점
    
    최적: 160~180 → 15점 (전 구간 만점)
    양호: 140~160, 180~200 → 12~15점
    보통: 100~140, 200~250 → 5~12점
    위험: 그 외 → 0~5점
    """
    if cci is None:
        return 7.5  # 중간값
    
    # v5.1: 최적 구간 확대 (160~180 전체 만점)
    if 160 <= cci <= 180:
        return 15.0  # 전 구간 만점!
    
    # 양호 구간 (140~160, 180~200)
    elif 140 <= cci < 160:
        return 12.0 + ((cci - 140) / 20) * 3.0  # 12~15점
    elif 180 < cci <= 200:
        return 15.0 - ((cci - 180) / 20) * 3.0  # 15~12점
    
    # 보통 구간 (100~140, 200~250)
    elif 100 <= cci < 140:
        return 5.0 + ((cci - 100) / 40) * 7.0  # 5~12점
    elif 200 < cci <= 250:
        return 12.0 - ((cci - 200) / 50) * 7.0  # 12~5점
...
def calc_consec_score(consec_days: int) -> float:
    ...
    score_map = {
        ...
        2: 15.0,  # 2일차 (최적!)
        3: 15.0,  # 3일차 (최적!)
        4: 13.0,  # 4일차 (v5.1: 12→13)
        5: 7.0,   # 5일차 (v5.1: 8→7, 강한 감점)
        6: 3.0,   # 6일차 (v5.1: 5→3, 강한 감점)
        7: 1.0,   # 7일차 (v5.1: 3→1, 강한 감점)
    }
    
    if consec_days in score_map:
        return score_map[consec_days]
    elif consec_days > 7:
        return 0.0  # v5.1: 8일 이상 0점
    else:
        return 8.0  # 기본값
...
def format_discord_embed(scores: List[StockScoreV5], title: str = "종가매매 TOP5") -> dict:
    ...
    return {
        ...
        "fields": fields,
        "footer": {
            "text": "v5.1 | 그리드서치 최적 조건 기반 | 100점 만점"
        }
    }
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("v5.1 점수 계산기 테스트")
    print("=" * 60)
    
    print("\n[CCI 점수 테스트] (15점 만점) - v5.1 개선")
