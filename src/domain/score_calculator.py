"""
점수 산출기 v5.4 - 백테스트 최적화 버전

📊 백테스트 결과 기반 최적 조건 (2016-2026, 6,999건)
===========================================
🏆 D+1 시초가 매도 승률: 54.5%
🏆 최적 CCI 구간: 150~170 (55.5%)
🏆 최적 연속양봉: 2~3일 (54.9%)
⚠️ 연속양봉 4일: 50.9% (급락!)
...
🔧 v5.4 변경사항 (백테스트 기반):
  - CCI 구간: 160~180 → 150~170 (승률 +0.4%p)
  - 연속양봉 4일: 13점 → 5점 (승률 50.9% 급락 반영)
  - 연속양봉 5일+: 7점 → 0~2점
  - K값 전략 제거 (승률 49% 부적합)
  - 글로벌 지표 필터 추가 (나스닥/환율)
"""
...
def calc_cci_score(cci: float) -> float:
    ...
    # 양호 구간 (130~150, 170~190)
    elif 130 <= cci < 150:
        return 12.0 + ((cci - 130) / 20) * 3.0  # 12~15점
    elif 170 < cci <= 190:
...
def calc_consec_score(consec_days: int) -> float:
    ...
    v5.4 개선: 4일 이상 더 강한 감점 (백테스트 기반)
    - 백테스트 결과: 4일 50.9% (2~3일 대비 -4%p)
    
    최적: 2~3일 → 15점
    양호: 1일 → 12점
    위험: 4일 → 5점 (v5.4: 13→5, 강한 감점!)
    매우위험: 5일+ → 0~2점
    """
    # v5.4: 4일부터 강한 감점 (백테스트 기반)
    score_map = {
        0: 8.0,   # 오늘 음봉
        1: 12.0,  # 1일차
        2: 15.0,  # 2일차 (최적!)
        3: 15.0,  # 3일차 (최적!)
        4: 5.0,   # 4일차 (v5.4: 13→5, 백테스트 50.9% 급락)
        5: 2.0,   # 5일차 (v5.4: 7→2)
        6: 0.0,   # 6일차 (v5.4: 3→0)
        7: 0.0,   # 7일차 (v5.4: 1→0)
    }
